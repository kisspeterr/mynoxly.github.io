...
const AdminDashboard = () => {
  const { isAuthenticated, isAdmin, isReady, signOut, profile } = useAuth();
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState('coupons');

  // ✅ Jogosultságkezelés – csak akkor fusson, ha minden auth adat már be van töltve
  useEffect(() => {
    if (!isReady) return; // még tölt → ne dönts semmiről
  ...
  // ✅ Amíg tölt az auth állapot, mutassuk a "Jogosultság ellenőrzése" képernyőt
  if (!isReady) {
    return (
  ...
  // ✅ Ha bejelentkezett, de nem admin → unauthorized oldal
  if (isAuthenticated && !isAdmin) {
    return <UnauthorizedAccess />;
  }

  // ✅ Ha minden rendben → mutassuk az admin felületet
  return (
      ...
      <div className="container mx-auto max-w-7xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 md:mb-12">
          ...
          <div className="md:hidden self-end">
            <DropdownMenu>
              ...
              <DropdownMenuContent
                align="end"
                className="bg-black/90 border-purple-500/30 backdrop-blur-sm text-white"
              >
                ...
                <DropdownMenuItem asChild>
                  <Link
                    to="/admin/dashboard?tab=statistics"
        ...
        <div className="bg-black/30 border border-purple-500/30 rounded-xl p-4 md:p-6 shadow-2xl backdrop-blur-sm">
          <p className="text-lg md:text-xl text-gray-300 mb-2">
            Üdvözöllek, {profile?.first_name || 'Admin'}!
          </p>
          <p className="text-md text-gray-400 mb-6 md:mb-8">
            Szervezet:{' '}
            <span className="font-semibold text-cyan-300">
              {profile?.organization_name || 'Nincs beállítva'}
            </span>
          </p>
          ...
          >
            <TabsList className="grid w-full grid-cols-5 bg-gray-800/50 border border-gray-700/50 h-auto p-1">
              ...
              >
                ...
                <span className="hidden sm:inline">Kuponok</span>
              </TabsTrigger>
              <TabsTrigger
                value="events"
              ...
              <TabsTrigger
                value="usages"
                className="data-[state=active]:bg-green-600/50 data-[state=active]:text-white data-[state=active]:border-b-2 data-[state=active]:border-green-400 py-2 text-sm md:text-base"
              >
                <ListChecks className="h-4 w-4 mr-1 md:mr-2" />
                <span className="hidden sm:inline">Beváltások</span>
              </TabsTrigger>
              <TabsTrigger
                value="statistics"
                className="data-[state=active]:bg-pink-600/50 data-[state=active]:text-white data-[state=active]:border-b-2 data-[state=active]:border-pink-400 py-2 text-sm md:text-base"
              >
